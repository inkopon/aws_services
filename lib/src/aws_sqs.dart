...
import 'aws_request_builder.dart';
...
class AwsSqs {
  ...
  /// AWS SQS
  AwsSqs({AwsCredentials? credentials, http.Client? httpClient})
      : _credentials = credentials!,
        _httpClient = httpClient! {
    log('instanciando AwsSqs');
    assert(_credentials != null);
    assert(_httpClient != null);
  }

  /// Returns a new SQS queue, inheriting the properties of this instance.
  SqsQueue queue(String queueUrl) => SqsQueue(queueUrl, credentials: _credentials, httpClient: _httpClient);
  ...
  Future<SqsQueue> create({required String region, required String queueName, String maxSize = '1024', String retentionPeriod = '345600'}) async {
    ...
    final queueUrl = xml.findAllElements('QueueUrl').first.innerText;
...
class SqsQueue {
  ...
  Future<List<SqsMessage>> receiveMessage(int number, {int? waitSeconds}) async {
    ...
    return xml
        ...
        .map((XmlElement elem) => SqsMessage(
              elem.findElements('MessageId').first.innerText,
              elem.findElements('ReceiptHandle').first.innerText,
              elem.findElements('Body').first.innerText,
            ))
        .toList();
  }
  ...
  Future sendMessage(String body, {String? messageGroupId, String? messageDeduplicationId}) async {
    final parameters = <String, String>{'Action': 'SendMessage', 'MessageBody': body, 'Version': '2012-11-05'};
