...
String _queryComponent(String path) => Uri.encodeQueryComponent(path).replaceAll('+', '%20');
...
class AwsRequestBuilder {
  ...
  Future<AwsResponse> sendRequest() async {
    ...
    return AwsResponse(rs.statusCode, rs.reasonPhrase, rs.headers.toSimpleMap(), rs.bodyAsStream as Stream<List<int>>); //TODO: VERIFICAR
  }
  ...
  void _sign() {
    ...
    final canonicalQuery = queryKeys.map((s) => '${_queryComponent(s)}=${_queryComponent(uri?.queryParameters[s] ?? "")}').join('&');
    final canonicalHeaders = headers!.keys.map((key) => '${key.toLowerCase()}:${headers?[key]?.trim()}').toList()..sort();
    final signedHeaders = (headers!.keys.toList()..sort()).map((s) => s.toLowerCase()).join(';');

    final payloadHash = headers?['X-Amz-Content-Sha256'] ?? sha256.convert(body!).toString();
